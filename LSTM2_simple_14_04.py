...
#                                                             ],3*Data['ID'].unique().shape[0])]
    ...
    out_S = pd.DataFrame(index = range(startTime,endTime,dt) ,columns= Data['ID'].unique()) #+before_dt*dt
    out_T = pd.DataFrame(index = range(startTime,endTime,dt) ,columns= Data['ID'].unique()) #+before_dt*dt
    out_TOT = pd.DataFrame(index = range(startTime,endTime,dt) ,columns= Data['ID'].unique())#+before_dt*dt
...
#    out_TOT = out_TOT.apply(smoothing, axis=0)
    ...
    np.save((os.getcwd()+'/DataPickles/FrameT%s' %day),return_array)
    return return_array
...
for Data_d in splitData:
    ...
    if idBus.shape[0] != 13:
        print('wtf',Data_d['datum_nach'].dt.dayofweek.unique(),Data_d['datum_nach'].unique())
    else:        
        out= pd.DataFrame(index = range(startTime,endTime+num_next_time*nex_time_dt*dt,dt), columns = idBus)
...
#                out.loc[t,:] = out.loc[t_last,:]
    #            out.loc[t,Data_t['fahrzeug'].unique()] =  Data_t.groupby('fahrzeug').last()
        ...
        out_fin = np.append(out_fin,return_array,)   
#        return return_array    
        del(Data_t,Data_d,out,return_array)

pool = mp.Pool(ncoeur) 

#out = pool.map(get_out,splitData)#np.array(,np.float32)
pool.close()
pool.join() # 28  
...
with open(os.getcwd()+'/DataPickles/out.pickle', 'wb') as o:
    pickle.dump(out, o)
