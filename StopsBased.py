...
def isgood(day):
    ...
    Data_d = Data.loc[(Data['linie'] == Studied_line) & (Data['datum_nach'] == day),:]
...
Data = Data[~(Data['halt_diva_nach'] == 6034)]
Data = Data[~(Data['halt_diva_nach'] == 6035)]
Studied_line = 32
...
#                                                             ],3*Data['ID'].unique().shape[0])]
    ...
    out_S = pd.DataFrame(index = range(startTime,endTime,dt) ,columns= Data['ID'].unique().astype('float32')).astype('float32')#+before_dt*dt
    out_T = pd.DataFrame(index = range(startTime,endTime,dt) ,columns= Data['ID'].unique().astype('float32')).astype('float32')#+before_dt*dt
    out_TOT = pd.DataFrame(index = range(startTime,endTime,dt) ,columns= Data['ID'].unique().astype('float32')).astype('float32')#+before_dt*dt
...
#    out_TOT = out_TOT.apply(smoothing, axis=0)
    ...
    np.save((os.getcwd()+'/DataPickles/StopsFrameT%s' %day),return_array)
#    return return_array
...
num_next_time = 10
nex_time_dt = 1
startTime += before_dt*dt
endTime += before_dt*dt
#Data_d = splitData[0]
def get_out(Data_d):
#out_fin = []
#for Data_d in splitData:
    Data_d = Data_d.loc[Data_d['linie'] == Studied_line,:]
    
    Data_d['Tot_latness'] = Data_d['ist_ab_nach']-Data_d['soll_ab_nach']
    
        
    out= pd.DataFrame(index = range(startTime,endTime+num_next_time*nex_time_dt*dt,dt), columns = Data_d.loc[Data_d['linie'] == Studied_line,'ID'].unique()).astype('float32')
        
    t_last = startTime
    Data_t = Data_d.loc[(Data_d['ist_ab_nach'] < t_last),['Tot_latness','ID']]
    out.loc[t_last].update(Data_t.groupby('ID').last()['Tot_latness'])
    
    for t in range(startTime+dt,endTime+num_next_time*nex_time_dt*dt,dt):
        Data_t = Data_d.loc[(Data_d['ist_ab_nach'] > t_last) &(Data_d['ist_ab_nach'] <= t),['Tot_latness','ID']]
        out.loc[t,:].update(Data_t.groupby('ID').last()['Tot_latness'])
...
#    out = out.apply(smoothing, axis=0)
...
with open(os.getcwd()+'/DataPickles/Stopsout.pickle', 'wb') as o:
    pickle.dump(out, o)
